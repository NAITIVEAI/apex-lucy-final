...
# Recent notice PDF cache (apex_id -> pdf info)
...
def _normalize_notice_apex_id(apex_id: Optional[str]) -> str:
    if not apex_id:
        return ""
    return str(apex_id).strip().upper()


def record_notice_pdf(apex_id: str, url: str, name: str, display: str = "side") -> None:
    """Record a recent notice PDF for later attachment in Chainlit."""
    key = _normalize_notice_apex_id(apex_id)
    if not key or not url:
        return
    with _notice_pdf_lock:
        _notice_pdf_cache[key] = {
            "url": url,
            "name": name,
            "display": display,
            "ts": time.time(),
...
def consume_recent_notice_pdf(apex_id: Optional[str]) -> Optional[Dict[str, Any]]:
    """Return and clear the recent PDF for the given apex_id if fresh."""
    key = _normalize_notice_apex_id(apex_id)
    if not key:
        return None
    with _notice_pdf_lock:
        entry = _notice_pdf_cache.get(key)
        if not entry:
            return None
        age = time.time() - float(entry.get("ts", 0))
        if age > _NOTICE_MAX_AGE_SECONDS:
...
def _sanitize_storage_path(raw_path: str) -> str:
    """Normalize storage paths that may contain markdown or query fragments."""
    if not raw_path:
        return raw_path
    path = raw_path.strip()
    try:
        ...
        # If storage path is a markdown link, extract the actual target
        for _ in range(2):
            path = unquote(path)
...
def _looks_like_markdown_path(path: str) -> bool:
    if not path:
        return False
    lowered = path.lower()
    return any(ch in path for ch in "[]()") or "%5b" in lowered or "%5d" in lowered


def _sanitize_blob_url(blob_url: str) -> str:
    """Strip markdown artifacts from a blob URL path."""
    if not blob_url:
        return blob_url
    try:
        import re
        ...
        # If path still doesn't end with .pdf, try to extract filename
        if not path.lower().endswith(".pdf"):
            pdf_match = re.search(r"([A-Za-z0-9_-]+\\.pdf)", path, re.I)
            if pdf_match:
                ...
                # Force into container if missing
                if f"/{container_name.lower()}/" not in path.lower():
                    path = f"/{container_name}/{filename}"
                else:
                    # Replace last segment with filename
                    path = re.sub(r"/[^/]+$", f"/{filename}", path)
...
# Cache Dynamics entity fields so updates can map to valid field names.
_ENTITY_FIELDS_CACHE: Dict[str, Dict[str, Any]] = {}
_ENTITY_FIELDS_CACHE_TTL = 60 * 30  # 30 minutes

_FIELD_ALIASES = {
    # Address fields
    "new_address": ["new_address", "new_address1", "address1_line1", "address1_line2"],
    "new_address1": ["new_address1", "new_address", "address1_line1", "address1_line2"],
    "new_city": ["new_city", "address1_city", "city"],
    "new_stateorprovince": ["new_stateorprovince", "new_state", "address1_stateorprovince", "state"],
    "new_postalcode": ["new_postalcode", "new_zip", "address1_postalcode", "postalcode", "zip"],
    "new_state": ["new_state", "new_stateorprovince", "address1_stateorprovince", "state"],
    "new_zip": ["new_zip", "new_postalcode", "address1_postalcode", "postalcode", "zip"],
    # Reissue flag
    "new_reissuecheckrequested": ["new_reissuecheckrequested", "new_checkreissuerequest"],
    # Contact fields
    "new_email": ["new_email", "emailaddress1", "email"],
...
def _resolve_update_fields(entity: str, updates: Dict[str, Any]) -> Dict[str, Any]:
    """Map update fields to existing entity fields using aliases when needed."""
    if not updates:
        return {}
    fields = _get_entity_fields_cached(entity)
    if not fields:
        return updates
    ...
    for key, value in updates.items():
        ...
        if mapped:
            if mapped not in resolved:
                ...
                logger.info(f"🧭 Mapped update field {key} -> {mapped}")
        else:
            resolved[key] = value
...
def update_member_profile_sync(apex_id: str, field_updates: Dict[str, Any]) -> str:
    ...
    Allowed fields for update (preferred):
        - new_address: Street address
        - new_city: City
        - new_state: State
        - new_zip: ZIP/Postal code

    Legacy fields (for backward compatibility):
        - new_address: Street address (legacy)
        - new_state: State (legacy)
        - new_zip: ZIP code (legacy)
        - new_address1: Street address (alternate)
        - new_stateorprovince: State/Province (alternate)
        - new_postalcode: ZIP/Postal code (alternate)
    ...
    try:
        ...
        if not safe_updates:
            return json.dumps({
                "success": False,
                "error": "No valid fields to update. Allowed fields: address.",
...
@trace_function(name="dynamics.update_member_profile_fields")
def update_member_profile_fields_sync(
    apex_id: str,
    new_address: str | None = None,
    new_city: str | None = None,
    new_state: str | None = None,
    new_zip: str | None = None,
) -> str:
    """
    Strict profile update tool with explicit fields only.
    ...
    if new_address:
        field_updates["new_address"] = new_address
    if new_city:
        field_updates["new_city"] = new_city
    if new_state:
        field_updates["new_state"] = new_state
    if new_zip:
        field_updates["new_zip"] = new_zip
    if not field_updates:
        return json.dumps({
            "success": False,
            "error": "No valid fields provided. Allowed: new_address, new_city, new_state, new_zip."
        })

    return update_member_profile_sync(apex_id, field_updates)


@trace_function(name="dynamics.update_reissue_check_requested")
def update_reissue_check_requested_sync(apex_id: str, requested: bool) -> str:
    """
    Set the reissue check request flag for a class member.

    Field: new_reissuecheckrequested (Yes/No dropdown in Dynamics)
    """
    try:
        member_filter = f"new_apexid eq '{apex_id}'"
        ...
        updates = {"new_reissuecheckrequested": bool(requested)}
        updates = _resolve_update_fields("new_classmembers", updates)

        logger.info(
            f"Updating reissue check request for {apex_id}: {updates}"
        )
        update_result = update_entity_sync('new_classmembers', member_guid, updates)

        return json.dumps({
            "success": True,
            "message": "Reissue check request updated",
            "apex_id": apex_id,
        ...
        logger.error(f"Error updating reissue request for {apex_id}: {str(e)}", exc_info=True)
        return json.dumps({
            "success": False,
            "error": str(e)
        })
...
def setup_dynamics_functions():
    ...
    return [
        ...
        # NOTE: get_case_details_sync removed - unscoped lookup, use get_member_cases_sync instead
...
def send_handoff_notification_email_sync(
    ...
    try:
        ...
        disable_availability = os.getenv("DISABLE_TEAMS_AVAILABILITY_CHECK", "true").lower() == "true"

        if disable_availability:
            logger.info("Teams availability checks disabled; skipping presence check.")
        elif teams_functions_available and check_teams_availability_sync:
        ...
        if disable_availability or not availability_result.get("available"):
            # If no agents available or Teams presence check failed, proceed with webhook notification anyway
            if not disable_availability and "error" in availability_result and "403" in str(availability_result.get("error", "")):
                ...
                logger.info("Proceeding with webhook notification to all agents.")
                ...
                elif disable_availability:
                    agent_email = "support@apexclassaction.com"
...
def request_human_assistance_sync(apex_id: str, reason: Optional[str] = None, timeout: int = 30) -> str:
    ...
    """
    try:
        if os.getenv("DISABLE_TEAMS_AVAILABILITY_CHECK", "true").lower() == "true":
            logger.info("Interactive Teams availability check disabled; using direct handoff.")
            return send_handoff_notification_email_sync(
                apex_id=apex_id,
                conversation_id="",
                phone_number="",
                best_time="",
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {
                "success": True,
                "entity": entity_set,
                "message": "Entity exists but no sample data available. Use update_member_profile_fields_sync to update fields.",
...
def parse_agentic_update_request(request: str) -> Dict[str, Any]:
    ...
    if "address" in request_lower:
        ...
        if match:
            updates["new_address"] = match.group(1).strip()
            updates["new_city"] = match.group(2).strip()
            updates["new_state"] = match.group(3).upper()
            updates["new_zip"] = match.group(4).strip()
        else:
            # Try simpler pattern
            if " to " in request_lower:
                ...
                if parts:
                    updates["new_address"] = parts[0].strip()
                    if len(parts) > 1:
                        updates["new_city"] = parts[1].strip()
                    if len(parts) > 2:
                        state_zip = parts[2].strip().split()
                        if state_zip:
                            updates["new_state"] = state_zip[0].strip()
                        if len(state_zip) > 1:
                            updates["new_zip"] = state_zip[1].strip()
...
def find_notice_for_user_sync(apex_id: str) -> str:
    ...
    try:
        ...
        try:
            ...
            search_index_name = os.getenv("AZURE_SEARCH_INDEX_NAME", "lucy-notices-v2")
            ...
            def _update_progress(label: str, value: int) -> None:
                ...
                try:
                    element = cl.CustomElement(
                        name="ProgressBar",
                        props=build_progress_props(label, value),
                    )
                    if progress_message is None:
                        progress_message = cl.Message(content="", elements=[element])
                        _safe_async_run(progress_message.send())
                    else:
                        progress_message.elements = [element]
                        _safe_async_run(progress_message.update())
                except Exception as progress_error:
                    logger.debug(f"[Lucy] Progress update skipped: {progress_error}")

            _update_progress("Searching for your notice...", 10)

            def _filter_pdf_results(results):
                """Return only PDF results, defensively filtering indexer noise."""
                pdf_results = []
                for result in results:
                    storage_path = result.get("metadata_storage_path", "")
                    storage_name = result.get("metadata_storage_name", "")
                    file_ext = result.get("file_extension") or result.get("metadata_storage_file_extension") or ""
                    file_ext = file_ext.lower().strip()

                    # Check if file is a PDF by extension or metadata
            ...
            def _run_search(
                ...
                *,
                ...
                logger.info(f"[Lucy] Document search attempt: {label}")
                search_kwargs = {
                ...
                if search_fields:
                    # Azure SDK expects a list; passing a string can be split into characters.
                    if isinstance(search_fields, (list, tuple)):
                        search_kwargs["search_fields"] = list(search_fields)
                    else:
                        search_kwargs["search_fields"] = str(search_fields)

                filter_used = "filter" in search_kwargs
                try:
                    search_results = search_client.search(**search_kwargs)
                except (HttpResponseError, TypeError, ValueError) as search_err:
                    if isinstance(search_err, HttpResponseError):
                        logger.warning("⚠️ PDF filter failed; retrying search without filter", exc_info=True)
                        search_kwargs.pop("filter", None)
                        filter_used = False
                    else:
                        logger.warning(
                            "⚠️ Search parameters rejected; retrying without search_fields",
                            exc_info=True,
                        )
                        search_kwargs.pop("search_fields", None)
                    search_results = search_client.search(**search_kwargs)
                ...
                pdf_results = _filter_pdf_results(results_list)
                logger.info(f"[Lucy] Document search returned {len(pdf_results)} PDF results for {label}")
                return pdf_results
            ...
            def _build_extended_queries(member: Dict[str, Any]):
                if not member:
                    return []

                first = _clean_search_value(member.get("new_firstname", ""))
                last = _clean_search_value(member.get("new_lastname", ""))
                full = _clean_search_value(member.get("new_fullname", ""))
                name_simple = " ".join([p for p in [first, last] if p]).strip()

                names = []
                if full:
                    names.append(full)
                if name_simple and name_simple.lower() != full.lower():
                    names.append(name_simple)

                street = _clean_search_value(member.get("new_address", ""))
                city = _clean_search_value(member.get("new_city", ""))
                state = _clean_search_value(member.get("new_state", "")).upper()
                zip_code = _clean_search_value(member.get("new_zip", ""))

                def _quoted(value: str) -> str:
                    return f'"{value}"'

                def _add_query(label: str, parts: List[str], seen: set, out: List):
                    parts = [p for p in parts if p]
                    if not parts:
                        return
                    query = " AND ".join(_quoted(p) for p in parts)
                    if query in seen:
                        return
                    seen.add(query)
                ...
                seen_queries = set()
                for name in names:
                    _add_query("name_zip", [name, zip_code], seen_queries, queries)
                    _add_query("name_city_zip", [name, city, zip_code], seen_queries, queries)
                    address_parts = [name, street, city]
                    if state:
                        address_parts.append(state)
                    if zip_code:
                        address_parts.append(zip_code)
                    _add_query("name_full_address", address_parts, seen_queries, queries)
                    _add_query("name_only", [name], seen_queries, queries)

                return queries

            def _fetch_member_profile(apex_id_value: str) -> Optional[Dict[str, Any]]:
                if not DYNAMICS_ENABLED:
                    return None
                if not apex_id_value:
                    return None
                try:
                    escaped_apex_id = apex_id_value.upper().replace("'", "''")
                    ...
                    fields = _get_entity_fields_cached("new_classmembers")
                    if fields:
                        select_fields = [f for f in desired_fields if f in fields]
                    else:
                        select_fields = desired_fields

                    select = ",".join(select_fields) if select_fields else None
                    result_str = query_entity_sync("new_classmembers", filter_str=filter_str, select=select)
                    result = json.loads(result_str)
                    if isinstance(result, list) and result:
                        return result[0]
                except Exception as member_err:
                    logger.warning(f"[Lucy] Failed to load member profile for extended search: {member_err}")
                return None

            def _build_apex_name_query(member: Optional[Dict[str, Any]], apex_value: str) -> Optional[str]:
                if not member or not apex_value:
                    return None
                first = _clean_search_value(member.get("new_firstname", ""))
                last = _clean_search_value(member.get("new_lastname", ""))
                full = _clean_search_value(member.get("new_fullname", ""))
                name_value = full or " ".join([p for p in [first, last] if p]).strip()
                if not name_value:
                    return None
                return f'{apex_value} "{name_value}"'
            ...
            def _apply_apex_filter(
                ...
                *,
                ...
                filtered = filter_results_by_apex(
                    results,
                    apex_token,
                ...
                if filtered:
                    logger.info(f"[Lucy] {label}: {len(filtered)}/{original_count} results matched Apex ID")
                    return filtered
                logger.warning(f"[Lucy] {label}: no Apex ID matches")
                return []

            # Strategy 1: Apex ID filename match
            results_list = _run_search(
                f"\"{clean_apex_id}.pdf\"",
                "apex_id_filename",
            ...
            results_list = _apply_apex_filter(
                results_list,
                "apex_id_filename",
                require_match=True,
                allow_chunk=False,
            )
            _update_progress("Verifying notice ID...", 35)

            # Strategy 2: Apex ID text match (OCR/content)
            if not results_list:
                results_list = _run_search(
                    clean_apex_id,
                    "apex_id_content",
                ...
                _update_progress("Scanning notice text...", 60)

            # Strategy 3: Apex ID + member name (improves ranking vs noise)
            member_profile = None
            if not results_list:
                member_profile = _fetch_member_profile(apex_id)
                apex_name_query = _build_apex_name_query(member_profile, clean_apex_id)
                if apex_name_query:
                    results_list = _run_search(
                        apex_name_query,
                        "apex_id_name",
            ...
            if not results_list:
                if member_profile is None:
                    member_profile = _fetch_member_profile(apex_id)
                extended_queries = _build_extended_queries(member_profile)
                if extended_queries:
                    extended_search_used = True
                    for label, query in extended_queries:
                        results_list = _run_search(
                            query,
                            label,
                            search_fields=["chunk", "metadata_storage_name", "metadata_storage_path"],
            ...
            if results_list:
                for candidate in results_list:
                    if apex_matches_result(apex_token, candidate, allow_chunk=True):
                        doc = candidate
                        break
            if not doc and results_list:
                doc = results_list[0]
            ...
            storage_name = _sanitize_storage_path(doc.get("metadata_storage_name") or "")
            storage_name = storage_name.lstrip("/")

            def _path_matches_expected(expected_value: str, name_value: str, path_value: str) -> bool:
                if not expected_value:
                    return False
                expected = expected_value.lower()
                if name_value and name_value.lower().endswith(expected):
                    return True
                if path_value and path_value.lower().endswith(expected):
                    return True
                return False

            prefer_result_path = False
            if storage_name or storage_path:
                expected_filename = f"{clean_apex_id}.pdf" if clean_apex_id else ""
                prefer_result_path = extended_search_used or not _path_matches_expected(
                    expected_filename, storage_name, storage_path
                )
            ...
            if not candidate_paths:
                logger.error("No usable storage paths found in search result")
            ...
            for candidate in candidate_paths:
                if _looks_like_markdown_path(candidate):
                    continue
                blob_url = f"https://{storage_account}.blob.core.windows.net/{candidate}"
                logger.info(f"[Lucy] Candidate blob URL: {blob_url}")
                break

            if not blob_url:
                logger.error("Failed to build a clean blob URL")
                return "ERROR: Document path not found"
            ...
            _update_progress("Preparing secure link...", 90)

            # Use SAS URL for download link
            download_url = sas_url
            pdf_display_name = f"{clean_apex_id}.pdf"
            if storage_name and storage_name.lower().endswith(".pdf") and not _looks_like_markdown_path(storage_name):
                pdf_display_name = storage_name.split("/")[-1]
            elif storage_path:
                display_path = storage_path
                if display_path.lower().startswith("http"):
                    display_path = urllib.parse.urlparse(display_path).path
